    ...
    for i in range(m+1):
        for j in range(n+1):
            ...
            if i > 0 and j > 0 and  d[i-1][j-1] is not None:
                ...
                a = d[i-1][j-1][1].copy()
                if s[i-1] == t[j-1]:
                    cands.append([antcost, a])
                else:
                    a[(s[i-1], t[j-1])] += 1
                    cands.append([antcost+1, a])
            ...
            if j > 0 and d[i][j-1] is not None:
                ...
                a[('&', t[j-1])] += 1
                cands.append([antcost+1, a])
            d[i][j] = min(cands, key=lambda x: x[0])
    return d[m][n]
...
if __name__ == "__main__":
    '''
    for a in [['abt', 'ab'], ['afgt', 'afft'], ['uuuu', 'uuvuu']]: 
        print(a)
        print(levenshtein(a[0], a[1]))
    '''
    try:
        reffilename, testfilename = sys.argv[1:]
    except ValueError():
        sys.stderr.write("usage: cer.py referencefile youroutputfile")
        sys.exit(1)
